...
**PRICE_MAP 配置示例**:
```python
# 假设 fal 文档显示模型价格为 $0.025/次
PRICE_MAP = {
    "fal-ai/xxx/model-name": 10,  # $0.025 × 400 = 10 积分
}
```
...
API/MCP 请求 → TaskServiceV3.create_task()
    └─> v3_pricing.calculate_and_deduct()
        └─> executor.get_price()  # 从 PRICE_MAP 获取价格
...
**价格配置检查**:
```python
# 在执行器的 PRICE_MAP 中确认价格
# 换算规则：1 美元 = 400 积分
# 例如 fal 定价 $0.025/次 → 0.025 × 400 = 10 积分
...
- 执行器示例: 参考 `seedream_executor.py`（完整实现）
...
6. **测试验证**：
   - 模型信息: `GET /api/v3/models/{model_id}` 和 `get_model_info` 返回正确
   - 参数验证: 检查 params_schema 的类型、必填、枚举、默认值
   - API 任务: `POST /api/v3/tasks` 提交任务
   - MCP 任务: `submit_task` 工具调用
   - 扣费验证: 金额与 PRICE_MAP 一致
